...
import { ActivityIndicator, Linking, Modal, Pressable, ScrollView, Text, TextInput as NativeTextInput, View } from "react-native";
import { Activity, CalendarDays, Check, ChevronDown, ChevronLeft, ChevronRight, Circle, ClipboardCopy, ExternalLink, Instagram, Link2, MessageCircle, Minus, Pencil, Plus, Send, Trash2, X } from "lucide-react-native";
...
function TextInput(props: ComponentProps<typeof NativeTextInput>) {
  const { accessibilityLabel, value, onChangeText } = props;
  if (accessibilityLabel === "Program start date" && typeof value === "string" && onChangeText) {
    return <DatePickerField label="Start date" value={value} onChangeText={onChangeText} placeholder="Start YYYY-MM-DD" containerClassName="flex-1" inputClassName="font-serif text-base" labelClassName="font-serif font-bold tracking-widest text-moss" accessibilityLabel="Program start date" />;
  }
  if (accessibilityLabel === "Program end date" && typeof value === "string" && onChangeText) {
    return <DatePickerField label="End date" value={value} onChangeText={onChangeText} placeholder="End YYYY-MM-DD" containerClassName="flex-1" inputClassName="font-serif text-base" labelClassName="font-serif font-bold tracking-widest text-moss" accessibilityLabel="Program end date" />;
  }
  if (accessibilityLabel === "Workout date" && typeof value === "string" && onChangeText) {
    return <DatePickerField label="Workout date" value={value} onChangeText={onChangeText} placeholder="YYYY-MM-DD" containerClassName="w-full" inputClassName="font-serif text-base" labelClassName="font-serif font-bold tracking-widest text-moss" accessibilityLabel="Workout date" />;
  }
...
function CoachLiveLogControls({ program, selectedEntry, onAddWorkout, onProgramDeleted, onWeekDeleted, onDayDeleted }: CoachLiveLogControlsProps) {
  const { addWeek, duplicateWeek, duplicateDay, updateProgram, deleteProgram, updateWeek, deleteWeek, updateDay, deleteDay } = useProgramWorkspaceStore();
  ...
  async function duplicateSelectedWeek() {
    if (!selectedWeek) return;
    try {
      await duplicateWeek(program.id, selectedWeek.id);
      setFeedback(`${selectedWeek.name} duplicated at the end of the program.`);
    }
    catch (reason) {
      setFeedback(reason instanceof Error ? reason.message : "Could not duplicate this week.");
    }
  }

  async function duplicateSelectedWorkout() {
    if (!selectedEntry) return;
    try {
      await duplicateDay(program.id, selectedEntry.weekId, selectedEntry.day.id);
      setFeedback(`${selectedEntry.day.name} duplicated at the end of its week.`);
    }
    catch (reason) {
      setFeedback(reason instanceof Error ? reason.message : "Could not duplicate this workout.");
    }
  }
    ...
    <View className="border border-fog bg-paper">
      <View className="flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <View className="flex-row flex-wrap justify-end gap-2">
          ...
          <Pressable className="min-h-10 flex-row items-center gap-2 border border-fog bg-canvas px-3 py-2 disabled:opacity-40" onPress={() => void duplicateSelectedWeek()} disabled={!selectedWeek || program.weeks.length >= 52} accessibilityLabel="Duplicate selected program week">
            <ClipboardCopy size={16} color="#F5F7FB" />
            <Text className="font-serif text-sm font-bold text-ink">Copy week</Text>
          </Pressable>
          <Pressable className="min-h-10 flex-row items-center gap-2 border border-fog bg-canvas px-3 py-2 disabled:opacity-40" onPress={() => void duplicateSelectedWorkout()} disabled={!selectedEntry || (selectedWeek?.days.length ?? 7) >= 7} accessibilityLabel="Duplicate selected workout">
            <ClipboardCopy size={16} color="#F5F7FB" />
            <Text className="font-serif text-sm font-bold text-ink">Copy workout</Text>
          </Pressable>
...
export function TrainingLogScreen() {
  ...
  async function updateSet(exercise: ProgramDay["exercises"][number], setNumber: number, completionStatus: "pending" | "done" | "skipped") {
    ...
    const outcomeReason = completionStatus === "skipped" ? setResultDraft.outcomeReason || "failed" : undefined;
    ...
    try {
      await logDaySet(selectedProgram.id, selectedEntry.day.id, exercise.id, setNumber, completionStatus, completionStatus === "done" && exercise.prescriptionMode !== "exact" ? actualWeight : undefined, exercise.weightUnit, completionStatus === "pending" ? undefined : { actualRepetitions, actualRpe, meanVelocityMps, restSeconds, outcomeReason });
  ...
  return (
    <AppShell title="Training Log">
      <ScrollView
        className="flex-1"
        contentContainerClassName="mx-auto w-full max-w-6xl gap-6 px-4 py-6 pb-12"
        showsVerticalScrollIndicator={false}
      >
        <View className="flex-col gap-3 border-l-4 border-signal pl-4 sm:flex-row sm:items-end sm:justify-between">
          <View>
            <Text className="font-serif text-xs font-bold uppercase tracking-widest text-moss">
              {isCoach ? "Athlete training log" : "Your program"}
            </Text>
            <Text className="mt-2 font-serif text-3xl font-bold text-ink">
              {isCoach ? `${athlete.displayName}'s workouts` : "Training Log"}
            </Text>
            <Text className="mt-2 font-serif text-base text-[#52675F]">
              {isCoach
                ? "Navigate every workout, review actual loads and footage, then leave feedback in context."
                : "Every scheduled workout, actual set, video, rating, and coach note lives here."}
            </Text>
          </View>
          {selectedProgram ? (
            <View className="flex-row items-center gap-2">
              <CalendarDays size={18} color="#2E6F5E" />
              <Text className="font-serif text-sm font-bold text-ink">
                {scheduledDays.length} workout
                {scheduledDays.length === 1 ? "" : "s"}
              </Text>
            </View>
          ) : null}
        </View>
        {selectedEntry && selectedProgram ? (
          <TrainingLogSchedulePanel
            key={selectedEntry.day.id}
            programId={selectedProgram.id}
            weekId={selectedEntry.weekId}
            weekName={selectedEntry.weekName}
            day={selectedEntry.day}
            actorRole={session.role === "COACH" ? "coach" : "lifter"}
          />
        ) : null}
        {selectedEntry && !isCoach ? (
          <View className="border border-fog bg-paper p-4">
            <Text className="font-serif text-xs font-bold uppercase tracking-widest text-moss">
              Next set result
            </Text>
            <Text className="mt-1 font-serif text-sm text-muted">
              These values apply when you mark the next set done or skipped.
              Reps default to the prescription.
            </Text>
            <View className="mt-3 flex-row flex-wrap gap-2">
              <TextInput
                className="min-h-10 min-w-28 flex-1 border border-fog bg-canvas px-3 font-serif text-ink"
                value={setResultDraft.repetitions}
                onChangeText={(repetitions) =>
                  setSetResultDraft((draft) => ({ ...draft, repetitions }))
                }
              ...
              <TextInput
                className="min-h-10 min-w-28 flex-1 border border-fog bg-canvas px-3 font-serif text-ink"
                value={setResultDraft.rpe}
                onChangeText={(rpe) =>
                  setSetResultDraft((draft) => ({ ...draft, rpe }))
                }
                keyboardType="decimal-pad"
                placeholder="Actual RPE"
                placeholderTextColor="#8996AC"
                accessibilityLabel="Actual RPE"
              />
              <TextInput
                className="min-h-10 min-w-28 flex-1 border border-fog bg-canvas px-3 font-serif text-ink"
                value={setResultDraft.velocity}
                onChangeText={(velocity) =>
                  setSetResultDraft((draft) => ({ ...draft, velocity }))
                }
                keyboardType="decimal-pad"
                placeholder="Velocity m/s"
                placeholderTextColor="#8996AC"
                accessibilityLabel="Mean velocity metres per second"
              />
              <TextInput
                className="min-h-10 min-w-28 flex-1 border border-fog bg-canvas px-3 font-serif text-ink"
                value={setResultDraft.restSeconds}
                onChangeText={(restSeconds) =>
                  setSetResultDraft((draft) => ({ ...draft, restSeconds }))
                }
                keyboardType="number-pad"
            ...
            <Text className="mt-4 font-serif text-xs font-bold uppercase tracking-widest text-muted">
              If skipped, choose why
            </Text>
            <View className="mt-2 flex-row flex-wrap gap-2">
              {missedWorkReasons.map((reason) => (
                <Pressable
                  key={reason.value}
                  className={`border px-3 py-2 ${setResultDraft.outcomeReason === reason.value ? "border-signal bg-signal" : "border-fog bg-canvas"}`}
                  onPress={() =>
                    setSetResultDraft((draft) => ({
                      ...draft,
                      outcomeReason:
                        draft.outcomeReason === reason.value
                          ? ""
                          : reason.value,
                    }))
                  }
                  ...
                  accessibilityState={{
                    selected: setResultDraft.outcomeReason === reason.value,
                  }}
                >
                  <Text
                    className={`font-serif text-xs font-bold ${setResultDraft.outcomeReason === reason.value ? "text-white" : "text-ink"}`}
                  >
                    {reason.label}
                  </Text>
                </Pressable>
        ...
        {selectedEntry && selectedProgram && analysisTargets.length ? (
          <View className="border border-fog bg-paper p-4">
            <View className="flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <View className="flex-1">
                ...
                <Text className="mt-1 font-serif text-lg font-bold text-ink">
                  Analyze original lift footage
                </Text>
                <Text className="mt-1 font-serif text-sm leading-6 text-muted">
                  Browser-local AI pose analysis supports squat, bench, and
                  deadlift sets. Accessories are excluded. The source video is
                  never uploaded or saved.
                </Text>
              </View>
              <Pressable
                className="min-h-11 flex-row items-center justify-center gap-2 bg-ink px-4 py-3"
                onPress={() => setIsVideoAnalysisOpen(true)}
                accessibilityLabel="Analyze a lift video"
              >
                <Activity size={17} color="#FFFFFF" />
                <Text className="font-serif text-sm font-bold text-white">
                  Analyze lift
                </Text>
              </Pressable>
            </View>
            {savedAnalysisTargets.length ? (
              <View className="mt-4 flex-row flex-wrap gap-2 border-t border-fog pt-4">
                {savedAnalysisTargets.map((target) => (
                  <View
                    key={`${target.exerciseId}-${target.setNumber}`}
                    className="min-w-44 flex-1 border border-fog bg-canvas px-3 py-2"
                  >
                    <Text className="font-serif text-xs font-bold text-ink">
                      {target.exerciseName} · Set {target.setNumber}
                    </Text>
                    <Text className="mt-1 font-mono text-xs text-muted">
                      {analysisMetric(
                        target.videoAnalysis?.meanConcentricVelocityMps ?? null,
                        " m/s",
                      )}{" "}
                      · Est. RPE{" "}
                      {analysisMetric(
                        target.videoAnalysis?.confidence === "low"
                          ? null
                          : (target.videoAnalysis?.estimatedRpe ?? null),
                        "",
                      )}
                    </Text>
                  </View>
        ...
        {isAthletesCoach && selectedProgram ? (
          <CoachLiveLogControls
            program={selectedProgram}
            selectedEntry={selectedEntry}
            onAddWorkout={openWorkoutCreator}
            onProgramDeleted={() => {
              setSelectedProgramId(null);
              setSelectedDayId(null);
              setMessage("Live program removed.");
            }}
            onWeekDeleted={(weekId) => {
              if (selectedEntry?.weekId === weekId) setSelectedDayId(null);
              setMessage("Week removed from the live program.");
            }}
            onDayDeleted={(dayId) => {
              if (selectedEntry?.day.id === dayId) setSelectedDayId(null);
              setMessage("Workout removed from the live program.");
            }}
          />
        ) : null}
        {message ? (
          <View className="border border-moss bg-[#2E6F5E12] px-4 py-3">
            <Text className="font-serif text-sm text-moss">{message}</Text>
          </View>
        ) : null}
        {isLoading ? (
          <View className="items-center border border-fog bg-paper py-12">
            <ActivityIndicator color="#2E6F5E" />
            <Text className="mt-3 font-serif text-sm text-[#52675F]">
        ...
        {!isLoading && athletePrograms.length ? (
          <>
                ...
                {athletePrograms.map((program) => (
                  ...
                  >
                    <View className="flex-1 pr-2">
                      <Text
                        className={`font-serif text-sm font-bold ${selectedProgram?.id === program.id ? "text-white" : "text-ink"}`}
                      >
                        {program.name}
                      </Text>
                      <Text
                        className={`mt-1 font-serif text-xs ${selectedProgram?.id === program.id ? "text-[#FFFFFFCC]" : "text-[#52675F]"}`}
                      >
                        {program.phase} · {program.status}
                      </Text>
                    </View>
                      ...
                      <View className="flex-row gap-1">
                        ...
                        <Pressable
                          ...
                          onPress={(e) => {
                            ...
                            setMessage(
                              "Use the standard Coach Log Controls panel below to securely execute program deletions.",
                            );
                          }}
                        >
            ...
            {!selectedEntry && selectedProgram && isCoach ? (
              <View className="border border-fog bg-paper p-4">
                <View className="flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <View>
                    <Text className="font-serif text-xs font-bold uppercase tracking-widest text-moss">
                    ...
                    <Text className="mt-1 font-serif text-sm text-[#52675F]">
                      Create a dated day in{" "}
                      {selectedProgram.weeks[0]?.name ?? "the first week"}, then
                      add its prescriptions.
                    </Text>
                  </View>
                  {!isAddingWorkout ? (
                    <Pressable
                      className="min-h-10 flex-row items-center justify-center gap-2 rounded-md bg-ink px-3 py-2"
                      onPress={() => openWorkoutCreator()}
                      accessibilityLabel="Add the first workout"
                    >
                ...
                {isAddingWorkout ? (
                  <View className="mt-4 border-t border-fog pt-4">
                    <View className="flex-col gap-3 sm:flex-row">
                      <TextInput
                        ...
                        placeholder="Workout name, e.g. Day 1"
                    ...
                    <TextInput
                      className="mt-3 min-h-11 border border-fog bg-canvas px-3 font-serif text-base text-ink"
                      value={workoutDraft.focus}
                      onChangeText={(value) =>
                        setWorkoutDraft((draft) => ({ ...draft, focus: value }))
                      }
                      placeholder="Workout focus"
                      placeholderTextColor="#688078"
                      accessibilityLabel="Workout focus"
                    />
                    <View className="mt-3 flex-row justify-end gap-2">
                      <Pressable
                        className="rounded-md border border-fog px-3 py-2"
            ...
            {selectedEntry && selectedProgram ? (
              <>
                <View>
                  <View className="mb-2 flex-row items-center justify-between">
                    ...
                    <Text className="font-serif text-xs text-[#52675F]">
                      Week {selectedEntry.weekNumber} of{" "}
                  ...
                  >
                    {scheduledDays.map((entry) => (
                      <Pressable
                        key={entry.day.id}
                        className={`w-36 border p-3 ${entry.day.id === selectedEntry.day.id ? "border-ink bg-ink" : "border-fog bg-paper"}`}
                        onPress={() => selectDay(entry.day.id)}
                        accessibilityLabel={`Open ${entry.weekName}, ${entry.day.name}`}
                      >
                        <Text
                          className={`font-serif text-xs font-bold ${entry.day.id === selectedEntry.day.id ? "text-[#FFFFFFCC]" : "text-[#688078]"}`}
                        >
                          Week {entry.weekNumber}
                        </Text>
                        <Text
                          className={`mt-1 font-serif text-sm font-bold ${entry.day.id === selectedEntry.day.id ? "text-white" : "text-ink"}`}
                        >
                          {entry.day.name}
                        </Text>
                        <Text
                          className={`mt-1 font-serif text-xs ${entry.day.id === selectedEntry.day.id ? "text-[#FFFFFFCC]" : "text-[#52675F]"}`}
                        >
                          {formatDate(entry.day.scheduledDate)}
                        </Text>
                      </Pressable>
                    ))}
                  </ScrollView>
                </View>
                <View className="flex-col gap-4 border-y border-fog bg-canvas py-5 sm:flex-row sm:items-center sm:justify-between">
                  <View className="flex-1">
                    <Text className="font-serif text-xs font-bold uppercase tracking-widest text-moss">
                      {selectedEntry.weekName} ·{" "}
                      {formatDate(selectedEntry.day.scheduledDate)}
                    </Text>
                    <Text className="mt-1 font-serif text-2xl font-bold text-ink">
                      {selectedEntry.day.name}
                    </Text>
                    <Text className="mt-1 font-serif text-sm leading-6 text-[#52675F]">
                  ...
                  <View className="flex-row flex-wrap gap-2">
                    {isAthletesCoach ? (
                      <>
                        {(["squat", "bench", "deadlift"] as const).map(
                          (category) => (
                            <Pressable
                              key={category}
                              className="min-h-10 flex-row items-center gap-2 rounded-md bg-ink px-3 py-2"
                              onPress={() => void addPrimaryLift(category)}
                              accessibilityLabel={`Add ${category} to selected workout`}
                            >
                              <Plus size={16} color="#FFFFFF" />
                              <Text className="font-serif text-sm font-bold capitalize text-white">
                                {category}
                              </Text>
                            </Pressable>
                          ),
                        ...
                        <Pressable
                          className="min-h-10 flex-row items-center gap-2 rounded-md border border-fog bg-paper px-3 py-2"
                          onPress={openAccessoryCreator}
                          accessibilityLabel="Add accessory to selected workout"
                        >
                          <Plus size={16} color="#17212B" />
                          <Text className="font-serif text-sm font-bold text-ink">
                            Accessories
                          </Text>
                        </Pressable>
                    ...
                    <Pressable
                      className="h-10 w-10 items-center justify-center rounded-md border border-fog bg-paper disabled:opacity-40"
                      disabled={selectedDayIndex === 0}
                      onPress={() =>
                        selectDay(scheduledDays[selectedDayIndex - 1].day.id)
                      }
                      accessibilityLabel="Open previous training day"
                    >
                      <ChevronLeft size={18} color="#17212B" />
                    </Pressable>
                    <Pressable
                      className="h-10 w-10 items-center justify-center rounded-md border border-fog bg-paper disabled:opacity-40"
                      disabled={selectedDayIndex === scheduledDays.length - 1}
                      onPress={() =>
                        selectDay(scheduledDays[selectedDayIndex + 1].day.id)
                      }
                      accessibilityLabel="Open next training day"
                    >
                      <ChevronRight size={18} color="#17212B" />
                    </Pressable>
                  </View>
                </View>
                {isCoach && isAddingWorkout ? (
                  <View className="border border-fog bg-paper p-4">
                    <View className="flex-row items-center justify-between">
                      <View>
                        ...
                        <Text className="mt-1 font-serif text-lg font-bold text-ink">
                          Add to {selectedEntry.weekName}
                        </Text>
                      </View>
                      <Pressable
                        className="h-9 w-9 items-center justify-center rounded-md border border-fog"
                        onPress={() => setIsAddingWorkout(false)}
                        accessibilityLabel="Close new workout form"
                      >
                        <X size={17} color="#17212B" />
                      </Pressable>
                    </View>
                    <View className="mt-4 flex-col gap-3 sm:flex-row">
                      <TextInput
                        ...
                        placeholder="Workout name, e.g. Day 3"
                        placeholderTextColor="#688078"
                        accessibilityLabel="Workout name"
                      />
                      <DatePickerField
                  ...
                  <View className="border border-fog bg-paper p-4">
                    <View className="flex-row items-center justify-between">
                      <View>
                        ...
                        <Text className="mt-1 font-serif text-lg font-bold text-ink">
                          Add to {selectedEntry.day.name}
                        </Text>
                      </View>
                      <Pressable
                        className="h-9 w-9 items-center justify-center rounded-md border border-fog"
                        onPress={() => setIsAddingAccessory(false)}
                        accessibilityLabel="Close accessory form"
                      >
                        <X size={17} color="#17212B" />
                      </Pressable>
                    </View>
                    <TextInput
                      className="mt-4 min-h-11 border border-fog bg-canvas px-3 font-serif text-base text-ink"
                      value={accessoryDraft.name}
                      onChangeText={(value) =>
                        setAccessoryDraft((draft) => ({
                          ...draft,
                          name: value,
                        }))
                      }
                      placeholder="Accessory name, e.g. Chest-supported row"
                      placeholderTextColor="#688078"
                    ...
                    <View className="mt-3 flex-col gap-3 sm:flex-row">
                      <TextInput
                        ...
                        placeholder="Sets"
                        placeholderTextColor="#688078"
                        accessibilityLabel="Accessory sets"
                      />
                      <TextInput
                        className="min-h-11 flex-1 border border-fog bg-canvas px-3 font-serif text-base text-ink"
                        value={accessoryDraft.repetitions}
                        onChangeText={(value) =>
                          setAccessoryDraft((draft) => ({
                            ...draft,
                            repetitions: value,
                        ...
                        placeholder="Reps"
                        placeholderTextColor="#688078"
                        accessibilityLabel="Accessory repetitions"
                      />
                      <TextInput
                        className="min-h-11 flex-1 border border-fog bg-canvas px-3 font-serif text-base text-ink"
                        value={accessoryDraft.prescriptionValue}
                        onChangeText={(value) =>
                          setAccessoryDraft((draft) => ({
                            ...draft,
                        ...
                        placeholder="Target"
                        placeholderTextColor="#688078"
                        accessibilityLabel="Accessory prescription target"
                      />
                    </View>
                    <View className="mt-3 flex-row flex-wrap gap-2">
                      {(["rpe", "rir", "exact"] as const).map((mode) => (
                        <Pressable
                          key={mode}
                          className={`rounded-md border px-3 py-2 ${accessoryDraft.prescriptionMode === mode ? "border-ink bg-ink" : "border-fog bg-canvas"}`}
                          onPress={() =>
                            setAccessoryDraft((draft) => ({
                              ...draft,
                        ...
                        >
                          <Text
                            className={`font-serif text-sm font-bold ${accessoryDraft.prescriptionMode === mode ? "text-white" : "text-ink"}`}
                          >
                            {mode.toUpperCase()}
                          </Text>
                        </Pressable>
                      ...
                      {(["kg", "lb"] as const).map((unit) => (
                        <Pressable
                          key={unit}
                          className={`rounded-md border px-3 py-2 ${accessoryDraft.weightUnit === unit ? "border-moss bg-[#2E6F5E1A]" : "border-fog bg-canvas"}`}
                          onPress={() =>
                            setAccessoryDraft((draft) => ({
                              ...draft,
                              weightUnit: unit,
                  ...
                  <View className="mt-2 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <View>
                      <Text className="font-serif text-lg font-bold text-ink">
                        {dayLog?.sessionRating
                          ? `${dayLog.sessionRating} / 10`
                          : "Not rated"}
                      </Text>
                      <Text className="mt-1 font-serif text-sm text-[#52675F]">
                        {isCoach
                          ? "Athlete-reported effort for future stress management."
                          : "Rate how demanding this session felt after you train."}
                      </Text>
                    </View>
                    {isCoach ? null : (
                      <View className="flex-row flex-wrap gap-1.5">
                        {Array.from(
                          ...
                          <Pressable
                            key={rating}
                            className={`h-9 w-9 items-center justify-center rounded-md border ${dayLog?.sessionRating === rating ? "border-ink bg-ink" : "border-fog bg-canvas"}`}
                            onPress={() => void saveRating(rating)}
                            accessibilityLabel={`Rate this session ${rating} out of 10`}
                          >
                            <Text
                              className={`font-serif text-sm font-bold ${dayLog?.sessionRating === rating ? "text-white" : "text-ink"}`}
                            >
                              {rating}
                            </Text>
                          </Pressable>
                ...
                {selectedEntry.day.exercises.length ? (
                  selectedEntry.day.exercises.map((exercise) => (
                    <View
                      key={exercise.id}
                      className="border border-fog bg-paper"
                    >
                      <Pressable
                        className="flex-col gap-3 border-b border-fog px-4 py-4 sm:flex-row sm:items-center sm:justify-between"
                        onPress={() => toggleExercise(exercise.id)}
                        accessibilityLabel={`${collapsedExerciseIds.has(exercise.id) ? "Expand" : "Collapse"} ${exercise.name}`}
                      ...
                      >
                        ...
                        <View className="flex-row items-center gap-2">
                          ...
                          {isAthletesCoach ? (
                            <>
                              <Pressable
                                className="h-9 w-9 items-center justify-center rounded-md border border-fog bg-canvas"
                                onPress={(event) => {
                                  event.stopPropagation();
                                  beginExerciseEditing(exercise);
                                }}
                              ...
                              <Pressable
                                className="h-9 w-9 items-center justify-center rounded-md border border-signal bg-canvas"
                                onPress={(event) => {
                                  event.stopPropagation();
                                  setExercisePendingRemoval(exercise);
                                }}
                          ...
                          {collapsedExerciseIds.has(exercise.id) ? (
                            <ChevronRight size={18} color="#688078" />
                          ) : (
                            <ChevronDown size={18} color="#688078" />
                          )}
                        </View>
                      </Pressable>
                      {!collapsedExerciseIds.has(exercise.id) ? (
                      ...
                      exerciseDraft ? (
                        <View className="gap-3 border-b border-fog bg-canvas p-4">
                          <TextInput
                            className="min-h-11 border border-fog bg-paper px-3 font-serif text-base text-ink"
                          ...
                          <View className="flex-row flex-wrap gap-3">
                            <TextInput
                              className="min-h-11 min-w-24 flex-1 border border-fog bg-paper px-3 font-serif text-base text-ink"
                              value={exerciseDraft.sets}
                              onChangeText={(sets) =>
                                setExerciseDraft((draft) =>
                                  draft ? { ...draft, sets } : draft,
                                )
                              }
                              keyboardType="number-pad"
                              placeholder="Sets"
                              accessibilityLabel="Exercise sets"
                            />
                            <TextInput
                              className="min-h-11 min-w-24 flex-1 border border-fog bg-paper px-3 font-serif text-base text-ink"
                              ...
                              onChangeText={(repetitions) =>
                                setExerciseDraft((draft) =>
                                  draft ? { ...draft, repetitions } : draft,
                                )
                              }
                              keyboardType="number-pad"
                              placeholder="Reps"
                              accessibilityLabel="Exercise repetitions"
                            />
                            <TextInput
                              className="min-h-11 min-w-24 flex-1 border border-fog bg-paper px-3 font-serif text-base text-ink"
                              value={exerciseDraft.prescriptionValue}
                              onChangeText={(prescriptionValue) =>
                                setExerciseDraft((draft) =>
                                  draft
                                    ? { ...draft, prescriptionValue }
                                    : draft,
                                )
                              }
                              keyboardType="decimal-pad"
                              placeholder="Target"
                              accessibilityLabel="Exercise prescription target"
                            />
                          </View>
                          <View className="flex-row flex-wrap justify-between gap-2">
                            <View className="flex-row flex-wrap gap-2">
                              {(
                                ["rpe", "rir", "percent", "exact"] as const
                              ).map((mode) => (
                                <Pressable
                                  key={mode}
                                  className={`rounded-md border px-3 py-2 ${exerciseDraft.prescriptionMode === mode ? "border-ink bg-ink" : "border-fog bg-paper"}`}
                                  onPress={() =>
                                    setExerciseDraft((draft) =>
                                      draft
                                        ? { ...draft, prescriptionMode: mode }
                                        : draft,
                                    )
                                  }
                                >
                                  <Text
                                    className={`font-serif text-xs font-bold uppercase ${exerciseDraft.prescriptionMode === mode ? "text-white" : "text-ink"}`}
                                  >
                                    {mode}
                                  </Text>
                                </Pressable>
                      ...
                      {Array.from({ length: exercise.sets }, (_, index) => {
                        const setNumber = index + 1;
                        const setLog = getSetLog(
                          dayLog?.sets ?? [],
                          exercise.id,
                          setNumber,
                        );
                        const status = setLog?.completionStatus ?? "pending";
                        const weightKey = weightDraftKey(
                          exercise.id,
                          setNumber,
                        );
                        const actualWeight =
                          actualWeightDrafts[weightKey] ??
                          setLog?.actualWeight?.toString() ??
                          "";
                        const needsActualWeight =
                          exercise.prescriptionMode !== "exact";
                        return (
                          <View
                            key={setNumber}
                            className="gap-3 border-b border-fog px-4 py-4 last:border-b-0"
                          >
                            <View className="flex-row items-center gap-3">
                              <Text className="w-7 font-serif text-sm font-bold text-ink">
                                {setNumber}
                              </Text>
                              <View className="flex-1">
                                <Text className="font-serif text-sm font-bold text-ink">
                                ...
                                <Text className="mt-1 font-serif text-xs text-[#688078]">
                                  {status === "done"
                                    ? `Logged${setLog?.actualWeight ? ` · ${setLog.actualWeight} ${setLog.weightUnit}` : ""}`
                                    : status === "skipped"
                                      ? "Skipped"
                                      : needsActualWeight
                                        ? `Enter actual ${exercise.weightUnit} to complete`
                                        : "Exact prescribed load"}
                                </Text>
                              </View>
                              {isCoach ? (
                                <Text
                                  className={`font-serif text-xs font-bold ${status === "done" ? "text-moss" : status === "skipped" ? "text-signal" : "text-[#52675F]"}`}
                                >
                                  {status === "pending"
                                    ? "Pending"
                                    : status === "done"
                                ...
                                <View className="flex-row gap-2">
                                  <Pressable
                                    className={`h-9 w-9 items-center justify-center rounded-md border ${status === "done" ? "border-moss bg-moss" : "border-fog bg-paper"}`}
                                    onPress={() =>
                                      void updateSet(
                                        exercise,
                                        setNumber,
                                    ...
                                    accessibilityLabel={`${status === "done" ? "Clear" : "Mark"} set ${setNumber} done`}
                                  >
                                    <SetStatusIcon
                                      status={
                                  ...
                                  <Pressable
                                    className={`h-9 w-9 items-center justify-center rounded-md border ${status === "skipped" ? "border-signal bg-signal" : "border-fog bg-paper"}`}
                                    onPress={() =>
                                      void updateSet(
                                        exercise,
                                        setNumber,
                                    ...
                                    accessibilityLabel={`${status === "skipped" ? "Clear" : "Mark"} set ${setNumber} skipped`}
                                  >
                                    <SetStatusIcon
                                      status={
                                        status === "skipped"
                            ...
                            {!isCoach && needsActualWeight ? (
                              <TextInput
                                className="min-h-10 border border-fog bg-canvas px-3 font-serif text-base text-ink"
                                value={actualWeight}
                                onChangeText={(value) =>
                                  setActualWeightDrafts((drafts) => ({
                                    ...drafts,
                                    [weightKey]: value,
                                  }))
                                }
                                keyboardType="decimal-pad"
                                placeholder={`Actual weight in ${exercise.weightUnit}`}
                                placeholderTextColor="#688078"
                                accessibilityLabel={`Actual weight for set ${setNumber} of ${exercise.name}`}
                              />
                            ) : null}
                            <View className="flex-row items-center justify-between">
                              {setLog?.instagramVideoUrl ? (
                                <Pressable
                                  className="flex-row items-center gap-1.5"
                                  onPress={() =>
                                    Linking.openURL(setLog.instagramVideoUrl!)
                                  }
                                  accessibilityLabel={`Open Instagram video for set ${setNumber}`}
                                >
                                  <Instagram size={16} color="#D74F32" />
                                  <Text className="font-serif text-sm font-bold text-signal">
                                    Instagram linked
                                  </Text>
                                  <ExternalLink size={14} color="#D74F32" />
                                </Pressable>
                              ) : (
                                <Text className="font-serif text-sm text-[#688078]">
                                  No video linked
                                </Text>
                              )}
                              {isCoach ? null : (
                                <Pressable
                                  ...
                                  accessibilityLabel={`Add Instagram link for set ${setNumber}`}
                                >
                  ...
                  <View className="items-center border border-fog bg-paper px-5 py-10">
                    <Text className="font-serif text-base font-bold text-ink">
                      No exercises prescribed
                    </Text>
                    <Text className="mt-1 text-center font-serif text-sm text-[#52675F]">
                      {isCoach
                        ? "Add prescriptions from Programs before this training day begins."
                        : "Your coach has not added exercises to this day yet."}
                    </Text>
                  </View>
                )}
                <View className="border border-fog bg-paper p-4">
                  <View className="flex-row items-center gap-2">
                    <MessageCircle size={18} color="#2E6F5E" />
                    <View>
                      <Text className="font-serif text-xs font-bold uppercase tracking-widest text-moss">
                        Day comments
                      </Text>
                      <Text className="mt-1 font-serif text-lg font-bold text-ink">
                        Notes for {selectedEntry.day.name}
                      </Text>
                    </View>
                  </View>
                  <View className="mt-4 gap-3">
                    {selectedDayComments.length ? (
                      selectedDayComments.map((comment) => (
                        <View
                          key={comment.id}
                          className={`border px-3 py-3 ${comment.authorRole === "coach" ? "border-[#2E6F5E55] bg-[#2E6F5E0D]" : "border-fog bg-canvas"}`}
                        >
                          <View className="flex-row items-center justify-between gap-3">
                            <Text className="font-serif text-sm font-bold text-ink">
                              {comment.authorName}
                            </Text>
                            <Text className="font-serif text-xs text-[#688078]">
                              {comment.authorRole === "coach"
                                ? "Coach"
                                : "Athlete"}
                            </Text>
                          </View>
                          <Text className="mt-2 font-serif text-sm leading-6 text-[#52675F]">
                            {comment.body}
                          </Text>
                        </View>
                      ))
                    ) : (
                      <Text className="font-serif text-sm text-[#52675F]">
                        No notes on this training day yet.
                      </Text>
                    )}
                  </View>
                  <View className="mt-4">
                    <TextInput
                      className="min-h-20 border border-fog bg-canvas p-3 font-serif text-base text-ink"
                      value={commentDraft}
                      onChangeText={(value) =>
                        setCommentDrafts((drafts) => ({
                          ...drafts,
                          [selectedEntry.day.id]: value,
                        }))
                      }
                      placeholder={
                        isCoach
                          ? "Leave a coaching note for this day"
                          : "Leave a note for your coach"
                      }
                      placeholderTextColor="#688078"
                      multiline
                      textAlignVertical="top"
                      accessibilityLabel="Day comment"
                    />
                    <View className="mt-3 flex-row justify-end">
                      <Pressable
                        className="min-h-10 flex-row items-center gap-2 rounded-md bg-ink px-3 py-2 disabled:opacity-50"
                        onPress={() => void submitComment()}
                        disabled={!commentDraft.trim()}
                        accessibilityLabel="Post day comment"
                      >
              ...
              <View className="items-center border border-fog bg-paper px-5 py-12">
                <CalendarDays size={25} color="#688078" />
                <Text className="mt-3 font-serif text-base font-bold text-ink">
        ...
        {!isLoading && !athletePrograms.length ? (
          <View className="items-center border border-fog bg-paper px-5 py-12">
            ...
            <Text className="mt-1 text-center font-serif text-sm text-[#52675F]">
              {isCoach
                ? "Create a program for the selected athlete before opening logs."
      ...
      <Modal
        transparent
        animationType="fade"
        visible={exercisePendingRemoval !== null}
        onRequestClose={() => setExercisePendingRemoval(null)}
      >
        <View className="flex-1 items-center justify-center bg-black/60 px-5">
          <View className="w-full max-w-md border border-fog bg-paper p-5">
            <Text className="font-serif text-xs font-bold uppercase tracking-widest text-signal">
              Confirm removal
            </Text>
            <Text className="mt-2 font-heading text-2xl uppercase text-ink">
              Remove exercise?
            </Text>
            <Text className="mt-3 font-serif text-sm leading-6 text-muted">
              Remove {exercisePendingRemoval?.name ?? "this exercise"} and its
              set data from this workout? This action cannot be undone.
            </Text>
            <View className="mt-5 flex-row justify-end gap-2">
              <Pressable
                className="min-h-11 border border-fog px-4 py-3"
                onPress={() => setExercisePendingRemoval(null)}
              >
                <Text className="font-serif text-sm font-bold text-ink">
                  Cancel
                </Text>
              ...
              <Pressable
                className="min-h-11 bg-signal px-4 py-3"
                onPress={() => void confirmExerciseRemoval()}
              >
                <Text className="font-serif text-sm font-bold text-white">
                  Remove
                </Text>
      ...
      <InstagramLinkModal
        visible={instagramTarget !== null}
        exerciseName={
          instagramTarget
            ? `${instagramTarget.exerciseName} · set ${instagramTarget.setNumber}`
            : ""
        }
        onClose={() => setInstagramTarget(null)}
        onSave={saveInstagramLink}
      />
      <VideoAnalysisModal
        visible={isVideoAnalysisOpen}
        targets={analysisTargets}
        onClose={() => setIsVideoAnalysisOpen(false)}
        onSave={saveVideoAnalysis}
      />
    </AppShell>
  );
}