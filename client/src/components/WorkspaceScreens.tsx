  ...
  return (
    <AppShell title="Messages">
      <View className="flex-1 mx-auto w-full max-w-6xl px-4 py-6">
        <View className="mb-5 border-l-4 border-signal pl-4"><Text className="font-serif text-xs font-bold uppercase tracking-widest text-moss">{session?.role === "COACH" ? "Coach communication" : "Coach thread"}</Text><Text className="mt-2 font-serif text-3xl font-bold text-ink">{session?.role === "COACH" ? activeAthlete?.displayName ?? "No athlete selected" : "Coach conversation"}</Text><Text className="mt-2 font-serif text-base text-[#52675F]">Feedback stays attached to your current coaching context.</Text></View>
        <View className="flex-1 border border-fog bg-paper"><ScrollView className="flex-1" contentContainerClassName="gap-4 p-4" showsVerticalScrollIndicator={false}>{messages.length ? messages.map((message) => <View key={message.id} className={message.isCurrentUser ? "items-end" : "items-start"}><View className={`max-w-[86%] px-4 py-3 ${message.isCurrentUser ? "bg-ink" : "bg-canvas"}`}><Text className={`font-serif text-sm leading-6 ${message.isCurrentUser ? "text-white" : "text-ink"}`}>{message.body}</Text></View><Text className="mt-1 font-serif text-xs text-[#688078]">{message.author} · {message.timestamp}</Text></View>) : <View className="items-center py-10"><MessageCircle size={24} color="#688078" /><Text className="mt-3 font-serif text-sm font-bold text-ink">No messages yet</Text><Text className="mt-1 font-serif text-xs text-[#52675F]">Start a new coaching conversation below.</Text></View>}</ScrollView><View className="border-t border-fog p-3"><TextInput className="min-h-20 border border-fog bg-canvas p-3 font-serif text-base text-ink" multiline value={draft} onChangeText={setDraft} placeholder="Write a coaching note" placeholderTextColor="#688078" accessibilityLabel="Write a message" /><View className="mt-3 flex-row justify-between"><Text className="font-serif text-xs text-[#52675F]">Messages in this preview last for this session.</Text><Pressable className="h-10 w-10 items-center justify-center rounded-md bg-ink disabled:opacity-50" onPress={sendMessage} disabled={!draft.trim()} accessibilityLabel="Send message"><Send size={17} color="#FFFFFF" /></Pressable></View></View></View>
      </View>
    </AppShell>
  );
}
  ...
  return (
    <AppShell title="Athletes">
      <ScrollView className="flex-1" contentContainerClassName="mx-auto w-full max-w-6xl gap-7 px-4 py-6 pb-12" showsVerticalScrollIndicator={false}>
        ...
        <View className="border border-fog bg-paper">{athletes.length ? athletes.map((athlete, index) => { const insight = insights.find((item) => item.athleteId === athlete.id); const isSelected = activeAthlete?.id === athlete.id; return <Pressable key={athlete.id} className={`flex-col gap-4 px-4 py-5 active:bg-canvas sm:flex-row sm:items-center ${index ? "border-t border-fog" : ""}`} onPress={() => void selectAthlete(athlete.id)} accessibilityLabel={`Select ${athlete.displayName} for review`}><View className="flex-row items-center gap-3 sm:w-60"><View className={`h-11 w-11 items-center justify-center rounded-md ${isSelected ? "bg-signal" : "bg-moss"}`}><Text className="font-serif text-xs font-bold text-white">{athlete.initials}</Text></View><View><Text className="font-serif text-base font-bold text-ink">{athlete.displayName}</Text><Text className="font-serif text-xs text-[#52675F]">{athlete.competitionWeightClass ?? "Unspecified"} · {insight?.activeProgramName ?? "No active program"}</Text></View></View><View className="flex-row flex-1 justify-between"><View><Text className="font-serif text-xs text-[#688078]">Readiness</Text><Text className="font-serif text-lg font-bold text-ink">{insight?.readiness ?? "-"}</Text></View><View><Text className="font-serif text-xs text-[#688078]">Adherence</Text><Text className="font-serif text-lg font-bold text-ink">{insight?.adherencePercent === null || insight?.adherencePercent === undefined ? "-" : `${insight.adherencePercent}%`}</Text></View><View><Text className="font-serif text-xs text-[#688078]">Status</Text><Text className="font-serif text-sm font-bold text-moss">{insight?.syncStatus ?? "No program"}</Text></View></View><ChevronRight size={20} color="#52675F" /></Pressable>; }) : <View className="px-5 py-10"><Text className="font-serif text-base font-bold text-ink">No athletes assigned</Text><Text className="mt-1 font-serif text-sm text-[#52675F]">Invite an athlete from the dashboard to build your roster.</Text></View>}</View>
      </ScrollView>
    </AppShell>
  );
}
...
export function ProgramReviewScreen() {
  ...
  const insight = getCoachInsights(profiles, programs, dayLogs).find((item) => item.athleteId === activeAthlete?.id);
  const reviewItems = getCoachReviewItems(profiles, programs, dayLogs, comments).filter((item) => item.athleteId === activeAthlete?.id && !reviewedIds.includes(item.id));
  const progressPercent = insight?.adherencePercent ?? 0;

  return (
    <AppShell title="Program Review">
      <ScrollView className="flex-1" contentContainerClassName="mx-auto w-full max-w-6xl gap-7 px-4 py-6 pb-12" showsVerticalScrollIndicator={false}>
        <View className="flex-col gap-3 border-l-4 border-signal pl-4 sm:flex-row sm:items-end sm:justify-between"><View><Text className="font-serif text-xs font-bold uppercase tracking-widest text-moss">Active athlete</Text><Text className="mt-2 font-serif text-3xl font-bold text-ink">{activeAthlete?.displayName ?? "No athlete selected"}</Text><Text className="mt-2 font-serif text-base text-[#52675F]">{insight?.activeProgramName ?? "No active program"} · {activeAthlete?.upcomingMeet ?? "No meet scheduled"}</Text></View><Pressable className="min-h-10 flex-row items-center gap-2 rounded-md border border-fog bg-paper px-3 py-2" onPress={() => router.push("/athletes")}><Users size={16} color="#17212B" /><Text className="font-serif text-sm font-bold text-ink">Change athlete</Text></Pressable></View>
        <View className="flex-col gap-3 sm:flex-row"><View className="flex-1 border border-fog bg-paper p-4"><Text className="font-serif text-xs font-bold uppercase tracking-widest text-[#688078]">Readiness decision</Text><View className="mt-3 flex-row items-center gap-2">{insight?.readiness === null || insight?.readiness === undefined ? <CircleAlert size={20} color="#688078" /> : insight.readiness < 65 ? <TrendingDown size={20} color="#D74F32" /> : <TrendingUp size={20} color="#2E6F5E" />}<Text className="font-serif text-3xl font-bold text-ink">{insight?.readiness ?? "-"}</Text></View><Text className="mt-1 font-serif text-xs text-[#52675F]">{insight?.readiness === null || insight?.readiness === undefined ? "Assign a program to calculate readiness" : insight.readiness < 65 ? "Reduce top-set exposure" : "Maintain planned exposure"}</Text></View><View className="flex-1 border border-fog bg-paper p-4"><Text className="font-serif text-xs font-bold uppercase tracking-widest text-[#688078]">Set completion</Text><Text className="mt-3 font-serif text-3xl font-bold text-ink">{insight?.adherencePercent === null || insight?.adherencePercent === undefined ? "-" : `${progressPercent}%`}</Text><Text className="mt-1 font-serif text-xs text-[#52675F]">{insight ? `${insight.completedSets} of ${insight.plannedSets} prescribed sets` : "No program assigned"}</Text></View><View className="flex-1 border border-fog bg-paper p-4"><Text className="font-serif text-xs font-bold uppercase tracking-widest text-[#688078]">Main lift baselines</Text><Text className="mt-3 font-serif text-xl font-bold text-ink">{activeAthlete?.squatOneRepMaxKg ?? "-"} / {activeAthlete?.benchOneRepMaxKg ?? "-"} / {activeAthlete?.deadliftOneRepMaxKg ?? "-"}</Text><Text className="mt-1 font-serif text-xs text-[#52675F]">Squat · bench · deadlift kg</Text></View></View>
        <View className="border border-fog bg-paper p-5"><Text className="font-serif text-xs font-bold uppercase tracking-widest text-moss">Program progress</Text><Text className="mt-1 font-serif text-xl font-bold text-ink">Completed prescribed sets</Text><View className="mt-5"><ProgressBar value={progressPercent} color="#2E6F5E" /></View><View className="mt-4 flex-row justify-between"><Text className="font-serif text-sm text-[#52675F]">{insight?.completedSets ?? 0} completed</Text><Text className="font-serif text-sm font-bold text-ink">{insight?.plannedSets ?? 0} prescribed</Text></View></View>
