...
function CoachDashboard() {
  ...
  return (
    <ScrollView className="flex-1" contentContainerClassName="mx-auto w-full max-w-6xl gap-7 px-4 py-6 pb-12" showsVerticalScrollIndicator={false}>
      ...
      <View className="border border-zinc bg-zinc/10 p-5"><View className="flex-row items-center gap-3"><Send size={20} color="#CCFF00" /><View className="flex-1"><Text className="font-heading text-lg uppercase text-ink">Add an athlete</Text><Text className="mt-1 font-sans text-sm text-muted">Create a secure registration link. Iron Forge will also email it when delivery is configured.</Text></View></View><View className="mt-4 flex-col gap-3 sm:flex-row"><TextInput className="min-h-12 flex-1 border border-fog bg-canvas px-3 font-sans text-base text-ink" value={inviteEmail} onChangeText={setInviteEmail} placeholder="athlete@email.com" placeholderTextColor="#9B9B95" keyboardType="email-address" autoCapitalize="none" accessibilityLabel="Athlete email address" /><Pressable className="min-h-12 flex-row items-center justify-center gap-2 bg-signal px-4 py-3" onPress={() => void sendInvite()}><Send size={17} color="#F4F4ED" /><Text className="font-heading text-sm uppercase text-white">Create invite</Text></Pressable></View>{inviteMessage ? <Text className="mt-3 font-sans text-sm text-ink">{inviteMessage}</Text> : null}{inviteLink ? <View className="mt-3 flex-col gap-3 border border-fog bg-canvas p-3 sm:flex-row sm:items-center"><View className="flex-1 flex-row items-start gap-2"><Link2 size={16} color="#CCFF00" /><Text selectable className="flex-1 font-mono text-xs leading-5 text-muted">{inviteLink}</Text></View><Pressable className="min-h-10 flex-row items-center justify-center gap-2 border border-signal px-3" onPress={() => void Linking.openURL(inviteLink)} accessibilityRole="link" accessibilityLabel="Open athlete registration link"><Link2 size={15} color="#CCFF00" /><Text className="font-heading text-xs uppercase text-ink">Open link</Text></Pressable></View> : null}</View>
